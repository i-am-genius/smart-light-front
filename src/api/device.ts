...
} from '../utils/cameraCaptureConfig'
...
export async function getCamRoiConfig(camChipId: string): Promise<CamRoiConfig> {
  return captureConfigToLegacyCameraCard(await getCamCaptureConfig(camChipId))
}
...
export async function saveCamRoiConfig(camChipId: string, payload: CamRoiConfig): Promise<CamRoiConfig> {
  return captureConfigToLegacyCameraCard(
    await saveCamCaptureConfig(camChipId, legacyCameraCardToCaptureConfig(camChipId, payload)),
  )
}
