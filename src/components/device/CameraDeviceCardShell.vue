...
const compatibilityRoiConfig = computed<CamRoiConfig>(() => {
  const targets = captureConfig.value?.targets || []
  return {
    camChipId: props.device.chipId,
    sliderLampChipId: captureConfig.value?.sliderLampChipId || '',
    configured: Boolean(captureConfig.value?.configured),
    rois: [1, 2, 3].map(index => {
      const target = targets.find(item => Number(item.index) === index)
      return {
        targetIndex: index,
    ...
    sliderPresets: Object.fromEntries([1, 2, 3].map(index => {
      const target = targets.find(item => Number(item.index) === index)
      return [String(index), target?.sliderMm || 0]
    })),
    sliderMoveTimes: Object.fromEntries([1, 2, 3].map(index => {
      const target = targets.find(item => Number(item.index) === index)
      return [String(index), target?.moveTimes || { slow: 0, normal: 0, fast: 0 }]
    })),
  }
})
