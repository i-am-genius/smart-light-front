  ...
  <div class="settings-card calibration-panel">
    ...
    <section v-if="calibration?.legacyMigrationRequired" class="legacy-card">
      ...
      <div class="migration-actions">
        <BaseSelect
          v-model="legacySourceSelection"
          :options="captureDeviceOptions"
          placeholder="选择原始拍摄设备"
        />
        <button
          type="button"
          class="btn-primary"
          :disabled="!legacySourceSelection || migrationLoading"
          @click="migrateLegacy"
        >
      ...
      <div class="workflow-grid">
        ...
        <section class="motor-card">
          <div class="section-title-row">
            ...
            <button type="button" class="text-btn" :disabled="!canTestPrediction" @click="testPrediction">
              测试拟合位置
            </button>
          </div>
...
import type {
  GarmentCaptureSourceKey,
  SourceAwareGarmentAimCalibrationStatus,
} from '../../api/garmentCalibration'
...
const captureDeviceOptions = computed(() => [
  ...
  ...cameraDevices.value.map(device => ({
    label: `Camera · ${deviceLabel(device)}`,
    value: cameraSourceKey(device.chipId),
  })),
])
...
const selectedSourceLabel = computed(() => {
  if (selectedCaptureDevice.value === 'PHONE') return '手机'
  return selectedCamera.value ? deviceLabel(selectedCamera.value) : selectedCaptureDevice.value.replace('CAMERA:', '')
})
const canCapture = computed(() => {
  if (!selectedLamp.value?.online) return false
  return selectedCaptureDevice.value === 'PHONE' || Boolean(selectedCamera.value?.online)
})
...
const canConfirmSample = computed(() => Boolean(
  canMoveLamp.value && calibration.value?.currentTargetValid && !calibration.value?.currentTargetSampled,
))
const canTestPrediction = computed(() => Boolean(
  canMoveLamp.value && calibration.value?.currentTargetValid && calibration.value?.modelReady,
))
...
async function loadCalibration(forceSync: boolean) {
  ...
  try {
    ...
    if (result.legacyMigrationRequired && !legacySourceSelection.value) {
      legacySourceSelection.value = phoneSourceKey()
    }
...
async function handlePhoneFileChange(event: Event) {
  ...
  input.value = ''
  if (!file || !selectedLampChipId.value) return
  captureLoading.value = true
  const previousRecognition = calibration.value?.currentRecognizedAt || ''
  try {
    await fabricRecognize(file, selectedLampChipId.value)
    await loadCalibration(false)
    if (calibration.value?.currentRecognizedAt === previousRecognition) {
      startRecognitionPoll(previousRecognition, '手机图片已上传，但等待识别点刷新超时')
    } else {
      captureLoading.value = false
    }
    ...
    captureLoading.value = false
    toast.show(getErrorMessage(error, '手机图片识别失败'), 'error')
  }
}
...
function startRecognitionPoll(previousRecognition: string, timeoutMessage: string) {
  ...
  recognitionPoll = setInterval(async () => {
    ...
    await loadCalibration(false)
    if (calibration.value?.currentRecognizedAt && calibration.value.currentRecognizedAt !== previousRecognition) {
      captureLoading.value = false
      stopRecognitionPoll()
    } else if (recognitionPollCount >= 24) {
      captureLoading.value = false
      stopRecognitionPoll()
      toast.show(timeoutMessage, 'error')
    }
  }, 2000)
}
...
async function confirmSample() {
  ...
  try {
    await sendArmPosition(selectedLampChipId.value, { ...position })
    calibration.value = await addSourceGarmentAimCalibrationSample(
...
async function testPrediction() {
  ...
  toast.show('已下发当前拍摄设备识别点的拟合位置', 'success')
}

async function clearSamples() {
  if (!selectedLampChipId.value || !calibration.value?.sampleCount) return
  if (!window.confirm(`确定清空 ${selectedSourceLabel.value} 在这盏 Lamp 上的标定样本吗？`)) return
  clearLoading.value = true
  try {
    calibration.value = await clearSourceGarmentAimCalibration(selectedLampChipId.value, selectedCaptureDevice.value)
    syncSuggestedPosition(calibration.value, true)
    toast.show('当前拍摄设备的标定数据已清空', 'success')
  } catch (error) {
...
async function migrateLegacy() {
  if (!selectedLampChipId.value || !legacySourceSelection.value) return
  migrationLoading.value = true
  try {
    const result = await migrateLegacyGarmentAimCalibration(selectedLampChipId.value, legacySourceSelection.value)
    selectedCaptureDevice.value = legacySourceSelection.value
    calibration.value = result
    toast.show(`旧版 ${result.sampleCount} 条标定数据已迁移到所选拍摄设备`, 'success')
  } catch (error) {
    toast.show(getErrorMessage(error, '迁移旧版标定数据失败'), 'error')
  } finally {
...
async function copyCalibration(overwrite: boolean) {
  if (!selectedLampChipId.value || !copyTargetLampIds.value.length) return
  copyLoading.value = true
  try {
    await copyGarmentAimCalibration(selectedLampChipId.value, {
      sourceKey: selectedCaptureDevice.value,
      targetLampChipIds: [...copyTargetLampIds.value],
      overwrite,
    })
    toast.show(`已把“${selectedSourceLabel.value}”标定复制到 ${copyTargetLampIds.value.length} 盏 Lamp`, 'success')
    copyTargetLampIds.value = []
  } catch (error) {
    const message = getErrorMessage(error, '复制标定失败')
    if (!overwrite && message.includes('已存在') && window.confirm(`${message}\n\n是否覆盖目标 Lamp 的同来源标定数据？`)) {
      copyLoading.value = false
      await copyCalibration(true)
      return
    }
...
<style scoped>
.calibration-panel { min-width: 0; }
.hidden-file-input { display: none; }
.panel-header,.section-title-row,.progress-head,.sample-list-head,.motor-label,.calibration-actions,.migration-actions { display:flex;align-items:center;justify-content:space-between;gap:12px; }
.panel-header,.sample-list-head { align-items:flex-start; }
.panel-desc { margin:6px 0 0;color:#64748b;font-size:13px;line-height:1.5; }
.model-badge,.sampled-badge { flex:none;padding:5px 9px;border-radius:999px;background:#f1f5f9;color:#64748b;font-size:12px;font-weight:800; }
.model-badge.ready,.sampled-badge { background:#dcfce7;color:#15803d; }
.device-pair-grid { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:16px; }
.device-pair-grid label>span { display:block;margin-bottom:6px;color:#64748b;font-size:12px;font-weight:700; }
.legacy-card,.calibration-progress,.target-card,.motor-card,.sample-list,.copy-card { margin-top:14px;padding:14px;border:1px solid #e2e8f0;border-radius:14px;background:#f8fafc; }
.legacy-card { border-color:#fde68a;background:#fffbeb; }
.legacy-card p { margin:5px 0 12px;color:#92400e;font-size:12px; }
.migration-actions { justify-content:flex-start; }
.migration-actions :deep(.base-select) { min-width:240px; }
.progress-head { align-items:flex-start; }
.progress-head span,.section-title-row small,.sample-list-head small { color:#64748b;font-size:12px;line-height:1.4; }
.progress-head span { max-width:66%;text-align:right; }
.progress-track { height:7px;margin-top:10px;overflow:hidden;border-radius:999px;background:#e2e8f0; }
.progress-track span { display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#38bdf8,#2563eb);transition:width 220ms ease; }
.coverage-row { display:flex;flex-wrap:wrap;gap:6px 12px;margin-top:9px;color:#64748b;font-size:11px; }
.workflow-grid { display:grid;grid-template-columns:minmax(180px,.7fr) minmax(280px,1.3fr);gap:12px; }
.section-title-row>div,.sample-list-head>div { display:flex;flex-direction:column;gap:3px; }
.target-plane { position:relative;height:150px;margin-top:12px;overflow:hidden;border-radius:12px;border:1px solid #cbd5e1;background:linear-gradient(rgba(148,163,184,.11) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,.11) 1px,transparent 1px),#fff;background-size:25% 25%; }
.axis-horizontal,.axis-vertical { position:absolute;background:rgba(59,130,246,.2); }
.axis-horizontal { left:0;right:0;top:50%;height:1px; }.axis-vertical { top:0;bottom:0;left:50%;width:1px; }
.target-dot { position:absolute;z-index:1;width:16px;height:16px;border:3px solid #fff;border-radius:50%;background:#ef4444;box-shadow:0 0 0 3px rgba(239,68,68,.2);transform:translate(-50%,-50%); }
.coordinate-text,.empty-hint { margin:9px 0 0;color:#475569;font-size:12px;text-align:center; }
.empty-hint { display:grid;min-height:150px;place-items:center;color:#94a3b8; }
.motor-row { margin-top:12px; }.motor-label { color:#475569;font-size:12px; }.motor-label strong { color:#2563eb; }
.motor-control { display:grid;grid-template-columns:34px minmax(80px,1fr) 34px 72px;gap:7px;align-items:center;margin-top:5px; }
.motor-control button,.text-btn { border:1px solid #bfdbfe;border-radius:9px;background:#eff6ff;color:#2563eb;cursor:pointer;font-weight:800; }
.motor-control button { min-height:34px; }.motor-control input[type='range'] { width:100%;accent-color:#2563eb; }
.motor-number { width:100%;min-width:0;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:9px;padding:7px;background:#fff;color:#0f172a; }
.text-btn { padding:6px 9px;font-size:11px; }.calibration-actions { justify-content:flex-end;margin-top:14px; }
.btn-primary,.btn-secondary,.btn-danger { min-height:38px;border:none;border-radius:10px;padding:8px 13px;cursor:pointer;font-weight:800; }
.btn-primary { color:#fff;background:linear-gradient(135deg,#3b82f6,#2563eb); }.btn-secondary { color:#2563eb;background:#eef4ff; }.btn-danger { color:#dc2626;background:#fff1f2; }
button:disabled,input:disabled { cursor:not-allowed;opacity:.55; }
.copy-target-grid { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:12px; }
.copy-target { display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid #e2e8f0;border-radius:10px;background:#fff;color:#475569;font-size:12px; }
.sample-row { display:grid;grid-template-columns:38px 1.2fr repeat(2,1fr);gap:8px;margin-top:8px;padding-top:8px;border-top:1px solid #e2e8f0;color:#475569;font-size:11px; }
:global(.app-container.night-mode) .legacy-card,:global(.app-container.night-mode) .calibration-progress,:global(.app-container.night-mode) .target-card,:global(.app-container.night-mode) .motor-card,:global(.app-container.night-mode) .sample-list,:global(.app-container.night-mode) .copy-card { border-color:rgba(148,163,184,.18);background:rgba(15,23,42,.58); }
:global(.app-container.night-mode) .target-plane,:global(.app-container.night-mode) .motor-number,:global(.app-container.night-mode) .copy-target { border-color:rgba(148,163,184,.28);background-color:rgba(30,41,59,.9);color:#e2e8f0; }
:global(.app-container.night-mode) .motor-label,:global(.app-container.night-mode) .coordinate-text,:global(.app-container.night-mode) .sample-row { color:rgba(203,213,225,.84); }
@media (max-width:768px) { .calibration-panel{padding:12px}.device-pair-grid,.workflow-grid,.copy-target-grid{grid-template-columns:1fr}.progress-head,.migration-actions{flex-direction:column;align-items:stretch;gap:6px}.progress-head span{max-width:none;text-align:left}.target-plane{height:130px}.sample-row{grid-template-columns:30px 1fr 1fr}.calibration-actions{flex-wrap:wrap} }
</style>
