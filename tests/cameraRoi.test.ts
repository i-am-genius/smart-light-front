...
describe('camera ROI helpers', () => {
  ...
  it('uses one slider preset and a dedicated slider lamp binding', () => {
    ...
    assert.match(globalConfig, /拍照 Pan/)
    assert.match(globalConfig, /roiDraft\.capturePan/)
    assert.match(globalConfig, /拍照 Tilt/)
    assert.match(globalConfig, /roiDraft\.captureTilt/)
